...
describe("cross-source market aggregation", () => {
  it("merges matching markets with provenance and confidence", async () => {
    const aggregator = new MarketAggregator({
      dexscreener: { search: async () => [pair("dexscreener", 1, 1000)] },
      geckoterminal: {
        searchPools: async () => [pair("geckoterminal", 1.02, 900)],
      },
    });
    ...
    expect(market?.confidence).toBeGreaterThan(0.7);
  ...
  it("returns partial results and records failed sources", async () => {
    ...
    const [market] = await aggregator.search("base", { network: "eth" });
    expect(market?.confidence).toBeLessThan(0.7);
