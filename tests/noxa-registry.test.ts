...
function launchLog(blockNumber = 100n, logIndex = 2, removed = false) {
  const topics = encodeEventTopics({
    ...
    args: { token, deployer, dexFactory },
  });
  const data = encodeAbiParameters(
    parseAbiParameters(
      "address pairToken, address pool, uint256 dexId, uint256 launchConfigId, uint256 positionId, uint256 restrictionsEndBlock, uint256 initialBuyAmount",
    ),
...
describe("NOXA token registry", () => {
  ...
  it("backfills in inclusive chunks and decodes TokenLaunched", async () => {
    const c = client({
      getLogs: vi
        ...
        .mockResolvedValueOnce([launchLog(103n)]),
    });
    const result = await createNoxaTokenRegistry(c, { chunkSize: 3n }).discover(
      { fromBlock: 100n, toBlock: 105n },
    );
    expect(
      c.getLogs.mock.calls.map((x: any[]) => [x[0].fromBlock, x[0].toBlock]),
    ).toEqual([
      [100n, 102n],
      [103n, 105n],
    ]);
    expect(result).toEqual([
      {
        id: `${blockHash}:${txHash}:2`,
        token,
        deployer,
        dexFactory,
        pairToken: NOXA_WETH_ADDRESS,
        pool,
        dexId: 7n,
        launchConfigId: 8n,
        positionId: 9n,
        restrictionsEndBlock: 110n,
        initialBuyAmount: 12n,
        blockNumber: 103n,
        blockHash,
        transactionHash: txHash,
        logIndex: 2,
  ...
  it("uses a confirmation-safe tip by default", async () => {
    ...
    await createNoxaTokenRegistry(c, { confirmations: 5n }).discover({
      fromBlock: 90n,
    });
    expect(c.getLogs).toHaveBeenCalledWith(
      expect.objectContaining({ toBlock: 95n }),
    );
  });
  ...
  it("retries transient log failures without skipping the chunk", async () => {
    ...
    const result = await createNoxaTokenRegistry(client({ getLogs }), {
      retries: 1,
      retryDelayMs: 10,
      sleep,
    }).discover({ fromBlock: 100n, toBlock: 100n });
    ...
    const c = client({
      getLogs: vi
        ...
        .mockResolvedValue([
          launchLog(),
          launchLog(),
          launchLog(100n, 3, true),
        ]),
    });
    expect(
      await createNoxaTokenRegistry(c).discover({
        fromBlock: 100n,
        toBlock: 100n,
      }),
    ).toHaveLength(1);
  ...
  it("verifies both factory registration and token launchFactory", async () => {
    ...
    const c = client({
      readContract: vi
        ...
        .mockResolvedValueOnce(NOXA_FACTORY_ADDRESS),
    });
