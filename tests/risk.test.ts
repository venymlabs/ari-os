...
describe("risk analytics", () => {
  it("scores multiple risk dimensions and communicates uncertainty", () => {
    const report = analyzeRisk(
      {
        token: {
          ...
          verified: false,
          isProxy: true,
          name: "USDT Official ✅ http://x.bad",
        },
        holders: [
          { address: "a", share: 0.55 },
          { address: "b", share: 0.2 },
        ],
        liquidityUsd: 4_000,
        ...
        sources: [
          { source: "one", priceUsd: 1 },
          { source: "two", priceUsd: 1.5 },
        ],
      },
      { now: Date.now() },
    );
    ...
    expect(report.factors.map((x) => x.code)).toEqual(
  ...
  it("returns bounded low risk and lower confidence when evidence is sparse", () => {
    const report = analyzeRisk({
      token: { address: "0xabc", verified: true },
      sources: [{ source: "one" }],
    });
