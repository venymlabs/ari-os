...
import { canonicalPolicy, policyHash, TradingControl, ReservationLedger, ProductionRiskEvaluator, type PolicyDocument, type TradeRequest } from "../src/execution/control/index.js";
...
describe("reservation ledger",()=>{
  ...
  it("commit is idempotent, rolling windows expire, and reconciliation replaces state",()=>{
    ...
    c.reconcile([{id:"old",at:99n,asset:"A",strategy:"s",amount:150n}]);
...
describe("production risk",()=>{
  const input=(x:any={})=>({now:1000n,chain:1n,account:"acct",router:"R",tokenIn:"A",tokenOut:"B",amountIn:40n,slippageBps:50n,quoteAt:995n,quoteBlock:10n,currentBlock:10n,quoteBlockHash:"h",canonicalBlockHash:"h",tokenBalance:100n,nativeBalance:20n,estimatedGasCost:5n,...x});
  const limits={chains:[1n],accounts:["acct"],routers:["R"],tokens:["A","B"],maxPerTrade:{A:50n},maxReservedPerToken:{A:70n},maxReservedAggregate:100n,maxSlippageBps:100n,maxQuoteAge:10n,maxQuoteBlocks:2n,nativeGasReserve:10n};
  it("fails closed for policy, canonical quote, balance, gas, and incomparable units",()=>{const r=new ProductionRiskEvaluator(limits);for(const [x,reason] of [[{router:"X"},"router_not_allowed"],[{slippageBps:101n},"slippage_exceeded"],[{quoteAt:989n},"quote_stale"],[{canonicalBlockHash:"x"},"quote_noncanonical"],[{tokenBalance:39n},"insufficient_token_balance"],[{nativeBalance:14n},"insufficient_gas_reserve"],[{tokenIn:"B"},"per_trade_unit_unconfigured"]] as any[])expect(r.evaluate(input(x),[]).reasons).toContain(reason)});
  it("never aggregates heterogeneous native token units",()=>{const r=new ProductionRiskEvaluator(limits);expect(r.evaluate(input(),[{id:"x",at:999n,asset:"A",strategy:"live",amount:35n}]).reasons).toContain("token_reserved_exposure_exceeded");expect(r.evaluate(input({amountIn:31n}),[{id:"x",at:999n,asset:"B",strategy:"live",amount:70n}]).reasons).not.toContain("aggregate_reserved_exposure_exceeded")});
  it("atomically prevents oversubscription across connections and restart",()=>{const d=mkdtempSync(join(tmpdir(),"risk-")),db=join(d,"x.db");try{const a=new ReservationLedger(db),b=new ReservationLedger(db);expect(a.reserveWithin({id:"a",at:1n,asset:"A",strategy:"s",amount:40n},{perAsset:70n,aggregate:100n})).toBe(true);expect(b.reserveWithin({id:"b",at:1n,asset:"A",strategy:"s",amount:40n},{perAsset:70n,aggregate:100n})).toBe(false);a.release("a");expect(b.reserveWithin({id:"b",at:1n,asset:"A",strategy:"s",amount:40n},{perAsset:70n,aggregate:100n})).toBe(true);expect(new ReservationLedger(db).has("b")).toBe(true)}finally{rmSync(d,{recursive:true,force:true})}});
  it("aggregates only normalized quote values across mixed tokens and decimals",()=>{const d=mkdtempSync(join(tmpdir(),"risk-units-")),db=join(d,"x.db");try{const a=new ReservationLedger(db),b=new ReservationLedger(db),usd={denomination:"USD",decimals:6};expect(a.reserveWithin({id:"usdc",at:1n,asset:"USDC",strategy:"s",amount:50_000_000n},{perAsset:100_000_000n,aggregate:{...usd,max:100_000_000n,value:50_000_000n,evidence:"usdc:1"}})).toBe(true);expect(b.reserveWithin({id:"weth",at:1n,asset:"WETH",strategy:"s",amount:25_000_000_000_000_000n},{perAsset:1_000_000_000_000_000_000n,aggregate:{...usd,max:100_000_000n,value:50_000_000n,evidence:"weth:2000"}})).toBe(true);expect(a.reserveWithin({id:"more",at:1n,asset:"USDC",strategy:"s",amount:1n},{perAsset:100_000_000n,aggregate:{...usd,max:100_000_000n,value:1n,evidence:"usdc:1"}})).toBe(false)}finally{rmSync(d,{recursive:true,force:true})}});
  it("fails closed when aggregate exposure is enabled without valuation evidence",()=>{const l=new ReservationLedger();expect(l.reserveWithin({id:"x",at:1n,asset:"A",strategy:"s",amount:1n},{perAsset:2n,aggregate:{denomination:"USD",decimals:6,max:2n,value:1n,evidence:""}})).toBe(false)});
  it("fails closed across connections when any active reservation has a different quote denomination",()=>{const d=mkdtempSync(join(tmpdir(),"risk-denom-")),db=join(d,"x.db");try{const a=new ReservationLedger(db),b=new ReservationLedger(db);expect(a.reserveWithin({id:"usd",at:1n,asset:"A",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"USD",decimals:6,max:10n,value:1n,evidence:"usd"}})).toBe(true);expect(b.reserveWithin({id:"eur",at:1n,asset:"B",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"EUR",decimals:6,max:10n,value:1n,evidence:"eur"}})).toBe(false)}finally{rmSync(d,{recursive:true,force:true})}});
  it("fails closed across connections when any active reservation has different quote precision",()=>{const d=mkdtempSync(join(tmpdir(),"risk-precision-")),db=join(d,"x.db");try{const a=new ReservationLedger(db),b=new ReservationLedger(db);expect(a.reserveWithin({id:"six",at:1n,asset:"A",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"USD",decimals:6,max:10n,value:1n,evidence:"six"}})).toBe(true);expect(b.reserveWithin({id:"eight",at:1n,asset:"B",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"USD",decimals:8,max:10n,value:1n,evidence:"eight"}})).toBe(false)}finally{rmSync(d,{recursive:true,force:true})}});
  it("blocks aggregate reservations while an unvalued migration row is active",()=>{const d=mkdtempSync(join(tmpdir(),"risk-migration-")),db=join(d,"x.db");try{const legacy=new ReservationLedger(db,{reservationTtl:10n}),aggregate=new ReservationLedger(db,{reservationTtl:10n});expect(legacy.reserve({id:"legacy",at:1n,asset:"A",strategy:"s",amount:1n})).toBe(true);const request={id:"valued",asset:"B",strategy:"s",amount:1n};const limits={perAsset:10n,aggregate:{denomination:"USD",decimals:6,max:10n,value:1n,evidence:"value"}};expect(aggregate.reserveWithin({...request,at:2n},limits)).toBe(false);expect(aggregate.reserveWithin({...request,at:11n},limits)).toBe(true)}finally{rmSync(d,{recursive:true,force:true})}});
  it("enforces a configured ledger-wide aggregate quote unit",()=>{const l=new ReservationLedger(":memory:",{aggregateQuote:{denomination:"USD",decimals:6}});expect(l.reserveWithin({id:"eur",at:1n,asset:"A",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"EUR",decimals:6,max:10n,value:1n,evidence:"eur"}})).toBe(false);expect(l.reserveWithin({id:"usd8",at:1n,asset:"A",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"USD",decimals:8,max:10n,value:1n,evidence:"usd8"}})).toBe(false);expect(l.reserveWithin({id:"usd6",at:1n,asset:"A",strategy:"s",amount:1n},{perAsset:10n,aggregate:{denomination:"USD",decimals:6,max:10n,value:1n,evidence:"usd6"}})).toBe(true)});
});
