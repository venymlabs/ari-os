...
const services:CliServices={chat:vi.fn(async(x)=>({reply:x.message})),status:vi.fn(()=>({ok:true})),sessions:vi.fn(()=>[]),tools:vi.fn(()=>["quote"]),skills:vi.fn(()=>["risk"]),markets:vi.fn(()=>["ETH"]),simulate:vi.fn(x=>({safe:true,input:x})),jobs:vi.fn(()=>[])};
describe("CLI",()=>{
