...
const verifier: AuthVerifier = {
  verify: vi.fn(async (credential) =>
    credential === "valid" ? principal : null,
  ),
};
const pairing: PairingStore = {
  get: vi.fn(async (id: string) =>
    id === "paired"
      ? { deviceId: id, principalId: "user-1", state: "paired" as const }
      : null,
  ),
};
...
describe("gateway wire protocol", () => {
  it("validates strict typed request, response, and event frames and exports JSON schemas", () => {
    expect(requestFrameSchema.parse(request())).toMatchObject({
      kind: "request",
      id: "req-1",
    });
    expect(
    ...
    expect(() =>
      requestFrameSchema.parse({ ...request(), extra: true }),
    ).toThrow();
...
describe("identity, authorization, and isolation", () => {
  it("authenticates using the injected verifier and binds paired devices", async () => {
    const context = await core().authenticate({
      credential: "valid",
      deviceId: "paired",
    });
    expect(context.principal).toEqual(principal);
    expect(context.device?.state).toBe("paired");
    await expect(
      core().authenticate({ credential: "bad" }),
    ).rejects.toMatchObject({ code: "UNAUTHENTICATED" });
    await expect(
      core().authenticate({ credential: "valid", deviceId: "unknown" }),
  ...
  it("enforces roles, scopes, and capabilities without exposing signing authority", async () => {
    const gateway = core();
    const context = await gateway.authenticate({ credential: "valid" });
    expect(() =>
      gateway.authorize(context, {
        roles: ["operator"],
        scopes: ["sessions:write"],
        capabilities: ["chat"],
      }),
    ...
    expect(() =>
      gateway.authorize(context, { scopes: ["wallet:sign"] }),
  ...
  it("isolates sessions by principal and account", async () => {
    ...
    expect(() =>
      gateway.assertSessionAccess(
        { ...context, principal: { ...principal, id: "other" } },
        "ses_01ARZ3NDEKTSV4RRFFQ69G5FAV",
        "acct-a",
      ),
    ).toThrowError(/isolated/i);
    expect(() =>
      gateway.assertSessionAccess(
        context,
        "ses_01ARZ3NDEKTSV4RRFFQ69G5FAV",
...
describe("canonical identifiers and routing", () => {
  it("creates canonical sortable run and session IDs", () => {
    expect(canonicalRunId("01ARZ3NDEKTSV4RRFFQ69G5FAV")).toBe(
      "run_01ARZ3NDEKTSV4RRFFQ69G5FAV",
    );
    expect(canonicalSessionId("ses_01arz3ndektsv4rrffq69g5fav")).toBe(
      "ses_01ARZ3NDEKTSV4RRFFQ69G5FAV",
    );
  ...
  it("routes deterministically with thread > peer > account > agent precedence", () => {
    expect(
      routingKey({ agentId: "a", accountId: "z", peerId: "p", threadId: "t" }),
    ).toBe("thread:t");
    expect(routingKey({ agentId: "a", accountId: "z", peerId: "p" })).toBe(
      "peer:p",
    );
...
describe("transport-neutral request handling", () => {
  ...
  it("deduplicates idempotent requests within a principal and session", async () => {
    const gateway = core();
    let calls = 0;
    gateway.register("session.send", async () => ({ calls: ++calls }));
    const a = await gateway.handle(request(), { credential: "valid" });
    const b = await gateway.handle(request(), { credential: "valid" });
    expect(a).toEqual(b);
    expect(calls).toBe(1);
    await expect(
      gateway.handle(
        { ...request(), params: { message: "different" } },
        { credential: "valid" },
      ),
    ).rejects.toMatchObject({ code: "IDEMPOTENCY_CONFLICT" });
  });
  it("reports a safe health snapshot", () => {
    expect(core().health()).toMatchObject({
      status: "ok",
