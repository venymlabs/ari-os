...
describe("server",()=>{
 it("exposes lifecycle, metrics, build metadata and OpenAPI without authentication",async()=>{const app=createServer({ready:()=>true,health:async()=>({runtime:{status:"ok"}}),version:"1.2.3",build:"abc"});await app.ready();expect((await app.inject("/livez")).statusCode).toBe(200);expect((await app.inject("/readyz")).statusCode).toBe(200);expect((await app.inject("/metrics")).body).toContain("raos_ready 1");expect((await app.inject("/version")).json()).toMatchObject({version:"1.2.3",build:"abc",signing:false});expect((await app.inject("/openapi.json")).json()).toMatchObject({openapi:"3.1.0"});await app.close()});
