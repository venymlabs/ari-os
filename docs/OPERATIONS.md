...
- **Readiness down:** inspect dependency health and disk space; do not bypass it. `rpc: unhealthy` with a reachable endpoint means the cluster genesis hash did not match the configured network — check `RPC_URL` before anything else.
...
- **Provider outage/429:** allow bounded retry/backoff; do not switch clusters or weaken policy. A quote whose blockhash expired during the outage is dead — requote, never re-sign.
...
Run migrations as one explicit job before API. Compose worker/Telegram profiles are optional and must not be horizontally scaled while sharing SQLite. Containers support read-only root filesystems with only `/var/lib/raos` and `/tmp` writable. Systemd units use the `raos` account and `/etc/raos/raos.env`.
