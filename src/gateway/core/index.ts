...
const canonical = (prefix: "run" | "ses", value: string) => {
  const raw = value
    .toUpperCase()
...
export const canonicalRunId = (value?: string) =>
  canonical("run", value ?? randomBytes(16).toString("hex").slice(0, 26));
export const canonicalSessionId = (value?: string) =>
  canonical("ses", value ?? randomBytes(16).toString("hex").slice(0, 26));
...
export function routingKey(route: ChannelRoute) {
  ...
  throw new GatewayError(
    "INVALID_ROUTE",
    "route requires agent, account, peer, or thread",
  );
}
...
export class GatewayCore {
  ...
  private idempotency = new Map<
    string,
    { hash: string; response: ResponseFrame }
  >();
  ...
  negotiate(offered: readonly string[]) {
    const match = this.config.supportedVersions.find((v) =>
      offered.includes(v),
    );
    if (!match)
      throw new GatewayError(
        "VERSION_NOT_SUPPORTED",
        "no mutually supported gateway protocol version",
        { supported: this.config.supportedVersions },
      );
    return match;
  }
  async authenticate(input: AuthInput): Promise<GatewayContext> {
    ...
    if (!input.deviceId) return { principal };
    ...
    if (
      ...
      throw new GatewayError(
        "DEVICE_NOT_PAIRED",
        "device is not paired to this principal",
      );
    return { principal, device };
  }
  authorize(context: GatewayContext, requirements: AuthorizationRequirement) {
  ...
  claimSession(context: GatewayContext, sessionId: string, accountId: string) {
    ...
    if (
      ...
      throw new GatewayError(
        "SESSION_ISOLATED",
        "session isolated from principal/account",
      );
  ...
  async handle(raw: unknown, auth: AuthInput): Promise<ResponseFrame> {
    ...
    const hash = createHash("sha256")
      ...
      .digest("hex");
    const key = `${context.principal.id}:${request.sessionId ?? "-"}:${request.idempotencyKey}`,
