...
export type FinancialMessageKind =
  "approval" | "policy" | "simulation" | "transaction" | "receipt";
...
export type ContextSummarizer = (
  input: SummaryInput,
) => Promise<{ overview: string; references: SummaryReference[] }>;
...
const financialKinds = new Set<ContextMessage["kind"]>([
  "approval",
  "policy",
  "simulation",
  "transaction",
...
function redactText(value: string): string {
  let result = value;
  for (const [p, r] of secretPatterns) result = result.replace(p, r);
  return result;
}
  ...
  if (value && typeof value === "object")
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [
        key,
        /authorization|cookie|secret|token|password|api.?key|seed|private.?key/i.test(
          key,
        )
          ? "[REDACTED]"
          : redactUnknown(item),
...
function clone(message: ContextMessage): ContextMessage {
    ...
    if (current.role === "assistant" && current.toolCalls?.length) {
      ...
      for (let k = 0; k < callIds.length; k++) {
        ...
        if (
          ...
          throw new Error(
            "Invalid tool exchange: missing, delayed, or reordered result",
          );
        unit.push(i + 1 + k);
      }
...
export async function compileContext(
  ...
  const messages = input.map(clone),
  ...
  const head = Math.max(0, Math.floor(options.headMessages ?? 1)),
    tail = Math.max(0, Math.floor(options.tailMessages ?? 4));
  const initialTokens = estimates.reduce((a, b) => a + b, 0);
  const protectedIndexes = new Set<number>();
  for (const unit of allUnits) {
    const protectedUnit = unit.some(
      (i) =>
        i < head ||
  ...
  const requiredReferences: SummaryReference[] = pruned
  ...
  if (options.summarizer && pruned.length) {
    const raw = await summarizeSafely(
      options.summarizer,
      {
    ...
    if (
      raw &&
      typeof raw === "object" &&
      Array.isArray((raw as { references?: unknown }).references)
    ) {
      const requiredIds = new Set(requiredReferences.map((r) => r.messageId));
      const allowed = new Set(pruned.map((m) => m.id));
      const optional: SummaryReference[] = [];
      const used = new Set(requiredIds);
      for (const item of (raw as { references: unknown[] }).references)
  ...
  const finalTokens = compiled.reduce(
    (sum, m) => sum + validatedEstimate(baseEstimate, m),
    0,
  );
      ...
      requiredReferences
        ...
        .concat(
          messages
            .filter(
              (m) =>
                !removed.has(messages.indexOf(m)) &&
