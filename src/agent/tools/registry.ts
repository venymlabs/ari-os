...
export class ToolRegistry {
  ...
  defineToolset(name: string, tools: readonly string[]): this {
    for (const tool of tools)
      if (!this.#tools.has(tool)) throw new Error(`Unknown tool: ${tool}`);
  ...
  listPrivileged(
    filter: Omit<ToolFilter, "capabilities"> = {},
  ): ToolDefinition[] {
  ...
  async available(filter: ToolFilter = {}): Promise<ToolDefinition[]> {
    const checks = await Promise.all(
      this.list(filter).map(async (tool) => {
        try {
          return {
            tool,
  ...
  async invoke(
    ...
    if (!status.available)
      return reject(
        "UNAVAILABLE",
        status.reason ?? "Tool unavailable",
        undefined,
        tool,
      );
    ...
    const abort = (timeout: boolean) => {
      ...
      controller.abort(
        timeout ? new Error("Tool timed out") : context.signal?.reason,
      );
    ...
    try {
      ...
      const code: ToolError["code"] = controller.signal.aborted
        ? timeoutTriggered
          ? "TIMEOUT"
          : "CANCELLED"
        : "EXECUTION_ERROR";
  ...
  async invokeParallel(
    ...
    return Promise.all(
      calls.map((call, index) =>
        this.invoke(call.name, call.input, {
          ...context,
  ...
  async #emit(event: ToolAuditEvent) {
    try {
...
function isFailure(
  result: ToolResult,
): result is Extract<ToolResult, { ok: false }> {
  return !result.ok;
}
...
function toModelSchema(tool: ToolDefinition<unknown, unknown>): ModelSchema {
  return {
    name: tool.name,
