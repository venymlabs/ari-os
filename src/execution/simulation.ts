    ...
    type === "eip1559"
      ...
      : type === "eip2930"
        ? {
            ...common,
            type,
            gasPrice: t.gasPrice ?? 0n,
            accessList: t.accessList ?? [],
          }
        : { ...common, type: "legacy" as const, gasPrice: t.gasPrice ?? 0n };
  const serialized = serializeTransaction(transaction),
    transactionHash = keccak256(serialized);
  return { transaction: t, serialized, transactionHash, policyHash };
}
...
const evidenceBody = (
  ...
  domain: "robinhood-simulation-evidence/v1",
...
export function createSimulationEvidence(
  ...
  const body = evidenceBody(request, result);
  return {
    ...body,
    hash: keccak256(stringToHex(JSON.stringify(canonical(body)))),
...
export function simulationEvidenceHash(
  ...
  return keccak256(
    stringToHex(
      JSON.stringify(
        canonical({ domain: "robinhood-simulation-evidence/v1", ...e }),
      ),
    ),
  );
}
export function assertSimulationSafe(
  ...
  if (!result.success)
    throw Error(
      `simulation_reverted${result.revertReason ? `:${result.revertReason}` : ""}`,
    );
  if (!/^0x[0-9a-fA-F]{64}$/.test(result.blockHash ?? ""))
    throw Error("simulation_block_hash_invalid");
  if (
    result.transactionHash.toLowerCase() !==
    context.expectedTransactionHash.toLowerCase()
  )
    throw Error("simulation_transaction_mismatch");
  if (
    result.blockNumber > context.currentBlock ||
    context.currentBlock - result.blockNumber > context.maxBlockLag
  )
    throw Error("simulation_stale_block");
  ...
  for (const d of result.assetDeltas) {
    ...
    seen.add(k);
    if (!context.allowedAssets.has(k))
