...
import { analyzeRisk, type RiskInput } from "../risk/index.js";
...
export function createUtilityApi(c: Config): UtilityApi {
  const app = Fastify({ logger: false }) as unknown as UtilityApi;
  app.openapi = {
    openapi: "3.1.0",
  ...
  app.addHook("onRequest", async (req, reply) => {
    ...
    if (c.rateLimit) {
      const key = req.ip,
        ...
        x = hits.get(key);
      const h =
        !x || now - x.start >= c.rateLimit.windowMs
          ? { n: 1, start: now }
  ...
  const run = async <T>(p: Promise<T>, reply: any) => {
  ...
  const route = (path: string, schema: object, handler: any) => {
  ...
  route("/networks", {}, (_q: any, r: any) => run(c.source.networks(), r));
  route(
    "/search",
    {
      querystring: {
    ...
    (q: any, r: any) => run(c.source.search(q.query.q, q.query.network), r),
  );
  route(
    "/trending",
    { querystring: { ...qs, required: ["network"] } },
    async (q: any, r: any) =>
      run(
        c.source
          .trending(q.query.network)
          .then((rows) =>
            rows.map((x) => ({ ...x, trendingScore: scoreTrend(x) })),
          ),
        r,
      ),
  );
  route(
    "/pairs/new",
    { querystring: { ...qs, required: ["network"] } },
    (q: any, r: any) => run(c.source.newPairs(q.query.network), r),
  );
  const params = {
    type: "object",
    required: ["address"],
  ...
  route(
    "/tokens/:address",
    { params, querystring: { ...qs, required: ["network"] } },
    (q: any, r: any) =>
      run(c.source.token(q.query.network, q.params.address), r),
  );
  route(
    "/pairs/:address",
    { params, querystring: { ...qs, required: ["network"] } },
    (q: any, r: any) =>
      run(c.source.pair(q.query.network, q.params.address), r),
  );
  route(
    "/pairs/:address/ohlcv",
    { params, querystring: { ...qs, required: ["network", "period"] } },
    (q: any, r: any) =>
      run(
        c.source.ohlcv(
          q.query.network,
  ...
  route(
    "/pairs/:address/trades",
    { params, querystring: { ...qs, required: ["network"] } },
    (q: any, r: any) =>
      run(c.source.trades(q.query.network, q.params.address, q.query.limit), r),
  );
  route(
    ...
    (q: any, r: any) =>
      run(
        c.source.holders(q.query.network, q.params.address, q.query.limit),
        r,
      ),
  );
  route(
    ...
    (q: any, r: any) =>
      run(
        c.source.riskInput(q.query.network, q.params.address).then(analyzeRisk),
        r,
      ),
  );
  return app;
}
export function scoreTrend(x: Record<string, unknown>): number {
  const vol = Math.max(0, Number(x.volume24hUsd ?? 0)),
    liq = Math.max(0, Number(x.liquidityUsd ?? 0)),
    change = Math.abs(Number(x.priceChange24hPercent ?? 0));
  return (
    Math.round(
      Math.min(
        100,
