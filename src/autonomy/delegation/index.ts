...
export interface ChildRuntimeInput {
  ...
  deadline: number;
  signal: AbortSignal;
  metadata: { parentSessionId: string; depth: number };
}
...
const defaults: DelegationLimits = {
  maxDepth: 2,
  maxChildren: 4,
  maxIterations: 8,
  deadlineMs: 30_000,
...
const defaultCapabilities: CapabilityGrant[] = [
...
interface Entry {
  ...
  resolve: (r: DelegationResult) => void;
  settled: boolean;
  counted: boolean;
}
const terminal = (s: DelegationStatus) =>
  s === "completed" || s === "failed" || s === "cancelled";
const errorText = (e: unknown) => (e instanceof Error ? e.message : String(e));
function deepFreeze<T>(v: T, seen = new WeakSet<object>()): T {
  if (v && typeof v === "object" && !seen.has(v as object)) {
    seen.add(v as object);
    for (const x of Object.values(v as object)) deepFreeze(x, seen);
    Object.freeze(v);
  }
  return v;
}
function boundary<T>(v: T, label: string): T {
...
export class DelegationManager {
  readonly #options: DelegationOptions;
  ...
  spawn(request: SpawnRequest): DelegationHandle {
    ...
    const capabilities = request.requestedCapabilities.filter(
      (c) => request.parentCapabilities.includes(c) && this.#allowed.has(c),
    );
    if (!capabilities.length)
      throw new DelegationError("no permitted read-only capabilities");
    const safeRequest: SpawnRequest = {
      ...request,
      context: boundary(request.context, "context"),
      model: boundary(request.model, "model"),
      parentCapabilities: boundary(
        [...request.parentCapabilities],
        "capabilities",
      ),
      requestedCapabilities: boundary(capabilities, "capabilities"),
    };
    const id = `${request.parentSessionId}:child:${++this.#sequence}`;
    let resolve!: (r: DelegationResult) => void;
    const result = new Promise<DelegationResult>((r) => (resolve = r));
    const entry: Entry = {
      request: safeRequest,
      id,
      depth,
  ...
  cancelParent(parent: string, reason: unknown = "parent cancelled") {
  ...
  #cancel(e: Entry, reason: unknown) {
    if (terminal(e.status)) return;
  ...
  #drain() {
    while (this.#active < this.#limits.concurrency && this.#queue.length) {
      ...
      if (e.status === "queued") void this.#run(e);
    }
  }
  async #run(e: Entry) {
    this.#active++;
    ...
    try {
      const abort = new Promise<never>((_, reject) => {
        ...
        timer = setTimeout(
          () => e.controller.abort("deadline exceeded"),
      ...
      if (e.controller.signal.aborted) throw e.controller.signal.reason;
      if (
        !raw ||
      ...
      if (
        raw.iterations > this.#limits.maxIterations ||
        ...
        if (this.#options.verifyResult) {
          try {
            const timeout =
              this.#options.verifierTimeoutMs ?? this.#limits.deadlineMs;
            const ok = await Promise.race([
              ...
              new Promise<boolean>((_, r) =>
                setTimeout(() => r(new Error("verifier timeout")), timeout),
              ),
            ]);
            if (!ok) {
              await this.#finish(e, {
  ...
  async #finish(
    e: Entry,
    partial: Pick<DelegationResult, "status"> & Partial<DelegationResult>,
    announce = false,
  ) {
    if (e.settled) return;
    e.settled = true;
    ...
    if (e.counted) {
      e.counted = false;
      this.#counts.set(
        e.request.parentSessionId,
        Math.max(0, (this.#counts.get(e.request.parentSessionId) ?? 1) - 1),
      );
    }
