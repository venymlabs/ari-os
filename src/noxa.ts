...
export const NOXA_FACTORY_ADDRESS =
  "0xD9eC2db5f3D1b236843925949fe5bd8a3836FCcB" as const;
export const NOXA_FACTORY_START_BLOCK = 61_688n;
export const NOXA_WETH_ADDRESS =
...
export const noxaTokenAbi = parseAbi([
...
export type TokenVerification =
  ...
  | {
      verified: false;
      reason:
        | "factory-record-missing"
        | "factory-record-token-mismatch"
        | "token-factory-mismatch";
      record: LaunchedTokenRecord;
    };
...
export function createNoxaTokenRegistry(
  client: NoxaPublicClient,
  ...
  if (confirmations < 0n)
    throw new RangeError("confirmations cannot be negative");
  ...
  async function discover(
    ...
    const latest =
      range.toBlock ?? (await retry(() => client.getBlockNumber()));
    const toBlock =
    ...
    for (let start = fromBlock; start <= toBlock; start += chunkSize) {
      ...
      for (const log of logs) {
        ...
        if (
          ...
          throw new Error(
            "TokenLaunched log is pending or missing canonical identifiers",
          );
        }
    ...
    return [...launches.values()].sort((a, b) =>
  ...
  async function getLaunchedToken(
    token: Address,
    ...
    return {
      token: getAddress(tuple[0]),
      deployer: getAddress(tuple[1]),
      pairedToken: getAddress(tuple[2]),
      positionManager: getAddress(tuple[3]),
      positionId: tuple[4],
      dexId: tuple[5],
      launchConfigId: tuple[6],
      restrictionsEndBlock: tuple[7],
      supply: tuple[8],
      isToken0: tuple[9],
      poolFee: tuple[10],
      exists: tuple[11],
      initialBuyAmount: tuple[12],
    };
  }
  ...
  async function verifyToken(token: Address): Promise<TokenVerification> {
    ...
    if (!record.exists)
      return { verified: false, reason: "factory-record-missing", record };
    if (!sameAddress(record.token, requested))
      return {
        verified: false,
        reason: "factory-record-token-mismatch",
        record,
      };
    ...
    if (!sameAddress(factory, NOXA_FACTORY_ADDRESS))
      return { verified: false, reason: "token-factory-mismatch", record };
